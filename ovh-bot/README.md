...
- 安装完成只以本次提交任务的终态或本次观察到的安装活动为准；服务器原本已安装相同系统时不会再瞬间误报完成。
- 安装进度全程显示 IP；安装完成后可点击“标记没中”。备注保存在
  `data/server_notes.json`，以后执行 `/servers` 会继续显示；每个备注按钮直接绑定对应服务器，可连续或同时标记多台，不依赖临时安装操作记录。服务器总览不显示清除按钮，需要清除时先选择对应服务器，再在操作页点击“清除没中备注”。
